...
submit_button.addEventListener("click",(e)=>{
    ...
    if(checkName()&&checkEmail()&&checkPhone() &&checkMessage()){
        ...
        if(localStorage.getItem("lastTimeSendingMessage")){
             if(date-localStorage.getItem("lastTimeSendingMessage")>6000000){
                ...
                alertMessage.innerHTML=`<span >عفوا , مسموح لك بارسال رسالة واحدة</span>`;
...
function sendEmail(){
    (function(){
        emailjs.init("pOO1PSEAeP8UWBNvl");
    })();
    let serviceID="service_qftt45g";//email service id
    let templateID="template_z5fnpfd";//email template id 
    let params={
        Name:Name.value,
        Email:Email.value,
        Phone:Phone.value,
    ...
    .then(function() {
        ...
        alertMessage.innerHTML=`<span >تم إرسال رسالتك بنجاح</span>`;
        submit_button.disabled = false;
        submit_button.innerHTML="إرسال";
        ...
        alertDiv.classList.add("error","active")
        alertMessage.innerHTML=`<span >تعذر إرسال الرسالة</span>`;
        interval= setTimeout(()=>{
        alertDiv.classList.remove("active","error");
